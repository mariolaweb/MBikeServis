<div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
    ...
    <form wire:submit.prevent="{{ $canSubmit ? 'save' : '' }}" class="grid gap-6 p-6 bg-white border rounded-2xl">
        ...
        {{-- Dodjela servisera (VIDLJIVO samo za admin/vlasnik/menadžer) --}}
        @if ($canAssign && $location)
            <div>
        ...
        <div class="flex justify-end">
            <button type="{{ $canSubmit ? 'submit' : 'button' }}"
                class="px-4 py-2 text-white bg-blue-600 rounded-xl hover:bg-blue-700">
                Sačuvaj
            </button>
        </div>
    </form>
